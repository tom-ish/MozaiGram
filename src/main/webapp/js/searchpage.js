...
function displayAccessButton(name, i) {
	...
	*/
	...
	addSpan.className = "jp-artist";
	addSpan.style="display: block;";
	addSpan.setAttribute("float", "right");
	addSpan.setAttribute("width", "35px");
	addSpan.textContent="+";
	addSpan.addEventListener('click', addFriendClick, false);
	
	$(innerDiv).append(innerA1Element);
	$(li).append(innerDiv);
	
	li.addEventListener('click', accessClick, false);
